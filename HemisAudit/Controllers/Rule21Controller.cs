...
namespace HemisAudit.Controllers
{
    ...
    public class Rule21Controller : Controller
    {
        ...
        public async Task<IActionResult> DownloadSavedExcel(int runId)
        {
            ...
            var bytes = _export.ExportRule21Excel(review.Summary);
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            var review = await LoadAuthorizedSavedRunAsync(runId, requireDownloadAccess: true);
            if (review == null) return RedirectToAction(nameof(Run), new { id = runId });
            var bytes = _export.ExportRule21Csv(review.Summary, false);
            return File(bytes, "text/csv", $"Rule21_Validation_Results_Run_{runId}.csv");
        }
        ...
        public async Task<IActionResult> DownloadSavedExceptionsCsv(int runId)
        {
            var review = await LoadAuthorizedSavedRunAsync(runId, requireDownloadAccess: true);
            if (review == null) return RedirectToAction(nameof(Run), new { id = runId });
            var bytes = _export.ExportRule21Csv(review.Summary, true);
            return File(bytes, "text/csv", $"Rule21_FLAGGED_Run_{runId}.csv");
        }
        ...
        public async Task<IActionResult> DownloadExcel([FromBody] Rule21ValidationSummary summary)
        {
            ...
            var bytes = _export.ExportRule21Excel(summary);
            return File(bytes, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        ...
        {
            summary = await ResolveExportSummaryAsync(summary);
            var bytes = _export.ExportRule21Csv(summary, false);
            return File(bytes, "text/csv", $"Rule21_Validation_Results_{Ts()}.csv");
        }
        ...
        {
            summary = await ResolveExportSummaryAsync(summary);
            var bytes = _export.ExportRule21Csv(summary, true);
            return File(bytes, "text/csv", $"Rule21_FLAGGED_{Ts()}.csv");
        }
